...
for (famesoek in series_list) {
    ...
    # Ensure 'Epo' exists
    if (!("Epo" %in% colnames(df))) {
        cat("Expected column 'Epo' not found for series:", series_name, "\n")
        next
    }

    # Extract Epoch (ms) and Value directly from Epo
    df <- df %>%
      mutate(
        Epoch_ms = sapply(Epo, function(x) x[1]),
        Value = sapply(Epo, function(x) x[2]),
        Series = series_name
      ) %>%
      select(Epoch_ms, Value, Series)
